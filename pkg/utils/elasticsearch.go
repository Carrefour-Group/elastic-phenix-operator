...
func (es *Elasticsearch) PingES() error {
...
func (es *Elasticsearch) existsIndex(indexName string) *bool {
...
func (es *Elasticsearch) existsTemplate(templateName string) *bool {
	response, err := es.Client.Indices.ExistsTemplate([]string{templateName})
	if err != nil {
...
func (es *Elasticsearch) getNumberOfReplicasAndShards(indexName string) (*int32, *int32) {
	response, err := esapi.IndicesGetSettingsRequest{Index: []string{indexName}}.Do(context.Background(), es.Client)
	if err != nil {
...
func (es *Elasticsearch) getProperties(indexName string) (*string, error) {
	no := false
	response, err := esapi.IndicesGetMappingRequest{Index: []string{indexName}, IncludeTypeName: &no}.Do(context.Background(), es.Client)
	if err != nil {
...
func (es *Elasticsearch) updateIndexReplicas(indexName string, numReplicas int32) (int, string, error) {
	settings := strings.NewReader(fmt.Sprintf(`{"index" : {"number_of_replicas" : %v}}`, numReplicas))
	response, err := esapi.IndicesPutSettingsRequest{Index: []string{indexName}, Body: settings}.Do(context.Background(), es.Client)
	if err != nil {
...
func (es *Elasticsearch) updateIndexMapping(indexName string, properties string) (int, string, error) {
	mappingReader := strings.NewReader(properties)
	no := false
	response, err := esapi.IndicesPutMappingRequest{Index: []string{indexName}, Body: mappingReader, IncludeTypeName: &no}.Do(context.Background(), es.Client)
	if err != nil {
...
func (es *Elasticsearch) CreateOrUpdateIndex(indexName string, model string) (*EsStatus, error) {
	exists := es.existsIndex(indexName)
	...
	if exists == nil {
		...
		if status, err := es.updateIndexSettings(indexName, model); status != nil || err != nil {
			return status, err
		}

		if status, err := es.updateIndexProperties(indexName, model); status != nil || err != nil {
			return status, err
		}
	...
	response, err := esapi.IndicesCreateRequest{Index: indexName, Body: strings.NewReader(model), IncludeTypeName: shouldIncludeTypeName}.Do(context.Background(), es.Client)
...
func (es *Elasticsearch) updateIndexSettings(indexName string, model string) (*EsStatus, error) {
	oldNumReplicas, oldNumShards := es.getNumberOfReplicasAndShards(indexName)
	...
	if isReplicasUpdated {
		...
		statusCode, responseStr, err := es.updateIndexReplicas(indexName, *numReplicas)
		if err != nil {
...
func (es *Elasticsearch) updateIndexProperties(indexName string, model string) (*EsStatus, error) {
	oldProperties, err := es.getProperties(indexName)
	...
	if arePropertiesUpdates {
		...
		statusCode, responseStr, err := es.updateIndexMapping(indexName, *properties)
		if err != nil {
...
func (es *Elasticsearch) DeleteTemplate(templateName string) error {
	exists := es.existsTemplate(templateName)
	if exists != nil && !*exists {
	...
	response, err := es.Client.Indices.DeleteTemplate(templateName)
