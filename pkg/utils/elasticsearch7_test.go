...
func TestElasticsearch7_CreateOrUpdatePipeline(t *testing.T) {
	ctx := context.Background()
	assert := assert.New(t)
	elasticsearch := buildElasticsearch(t)
	defer deleteAll(elasticsearch)
...
func TestElasticsearch7_DeletePipeline(t *testing.T) {
	ctx := context.Background()
	assert := assert.New(t)
	elasticsearch := buildElasticsearch(t)
	defer deleteAll(elasticsearch)
	...
	status, err = elasticsearch.CreateOrUpdateIndex(ctx, indexName, `{"settings":{"number_of_replicas": "3", "number_of_shards": "5", "default_pipeline": "my-pipeline"}, "mappings":{"_doc": {"properties":{"description":{"type":"keyword"}}}}}`)
	assert.Nil(err)
	assert.Equal("200", status.HttpCodeStatus)
	assert.Equal(StatusCreated, status.Status)
	assert.True(*elasticsearch.existsIndex(ctx, indexName))
